...
- [ ] **User sovereignty.** ✅ `test_abp_whitelist_sovereign_over_important` (settings-textarea whitelist → user-allow band 6 resolves over a feed `||w^$important` band 3) + `test_user_regex_blocks` (a `pfb_regex_list` pattern blocks, VIP) + `test_user_regex_beats_feed_important_allow` (a user regex is **band-sovereign** — band 5 beats a feed `@@…$important` band 4, name stays blocked; only the band-6 whitelist un-blocks it). The user regex is loaded as the band-5 `PRIO_USER_BLOCK` payload (`pfb_unbound.py` init REGEX section) — `$badfilter`-immune (never in the reconcile rule list) **and** sovereign over feed allows; the matcher side is unit-pinned by `TestUserRegexSovereign`. ☐ **hand-run:** the alerts **"add to whitelist" button** path (a second whitelist entry point — drive the same backend the button calls).
