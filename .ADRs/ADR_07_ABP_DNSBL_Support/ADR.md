...
- **Status:** **Accepted** (2026-06-15; implemented 2026-06-02) — ABP DNS semantics (`@@`, cross-feed `@@`, `$important`/`$badfilter`, regex block/allow + static cap, whitelist sovereignty, per-line/BOM/path detection) are covered by `tests/smoke/test_smoke_abp.py` + the ADR-21 cases, green on **CE 2.8 + Plus 26.03** (fanout run 27547011086). The few remaining hand-run items (runtime regex evict *timing*, DNSBL-IP-from-ABP populate) are maintainer-attested. (Originally Implemented — pending live smoke.)
- **Date:** 2026-06-02
...
7. **User intent is sovereign (maintainer).** User-provided block domains, user regex, the whitelist (settings textarea + alerts "add to whitelist" button), and the TOP1M whitelist must **never** be overridden by a feed rule, and are **never** removable by a feed `$badfilter`. They are treated as `$important` and `$badfilter`-immune. The user is the sovereign; lists are automated and cannot fit every use case, so the GUI's explicit overrides always win. Concretely, **user block domains = the DNSBL Group "Custom_List"** (per-group textarea + the alerts "add to DNSBL customlist" button); they enter the build as the synthetic `{alias}_custom` row, now tagged **provenance='user'** in the manifest so the Python build bands them at the sovereign **user-block band (5)** — beating any feed allow (`@@`/`@@$important`). (Previously they flowed through the feed path at band 1 — a feed `@@…$important` could override them; that gap is now fixed.) The whitelist (band 6) is the ultimate override, beating even a user block. *Semantic to confirm (tracked as [#37](https://github.com/andrebrait/pfBlockerNG/issues/37)):* whitelisting a domain should also REMOVE it from a user blacklist (Custom_List) — query-time precedence already makes the whitelist win, but the store-level reconciliation may not happen today.
...
- [ ] **User sovereignty.** ✅ `test_abp_whitelist_sovereign_over_important` (settings-textarea whitelist → user-allow band 6 resolves over a feed `||w^$important` band 3) + `test_user_regex_blocks` + `test_user_regex_beats_feed_important_allow` (a user regex is **band-sovereign** — band 5 beats a feed `@@…$important` band 4) + `test_custom_list_block_beats_feed_important_allow` (a DNSBL Group **Custom_List** entry — `{alias}_custom`, manifest provenance='user' — is band 5 and beats a feed `@@…$important`; the live proof of the PHP→Python provenance threading). All three user inputs (whitelist, Custom_List, regex) load at the sovereign user tier: regex via the init REGEX payload, Custom_List via the build's provenance tag (`_dnsbl_rule_band` / the non-ABP band path). Unit-pinned by `TestUserRegexSovereign` + `TestUserCustomListSovereign` (incl. the contrast: the SAME rules without the user tag let the feed `@@$important` win). ☐ **hand-run:** the alerts **"add to whitelist" button** path (a second whitelist entry point — drive the same backend the button calls).
...
- [ ] **ABP × DNSBL-TLD mode.** ☐ **hand-run**. The manifest/Python classifier handles both ABP and plain feeds in TLD mode; a live TLD-mode setup (enabling `pfb_pytld` + a TLD set with one ABP and one plain feed) is not yet wired into the harness. Confirm the ABP feed's domains build without CSV-mangled garbage from raw `||x^`/`@@`/`/re/`/`0.0.0.0 host` lines and the plain feed receives the same Python zone/data classification.
...
per-group plaintext summaries. The live PHP NDJSON parser still stages each feed;
the PHP widget-statistics finalizer consumes the resulting in-memory group counts
without re-reading feed bytes or interpreting the manifest.
