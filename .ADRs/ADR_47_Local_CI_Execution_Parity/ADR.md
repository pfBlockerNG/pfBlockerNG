...
- **Status:** **Implemented — pending the §8 multi-box live acceptance** (2026-06-27; proposed 2026-06-26; design resolved + adversarially verified 2026-06-27). **P1 landed** via PR #591; **P2–P5 landed** via PR #595 (rebase-merged 2026-06-27): `run-id.sh`/`select-box.sh`, `build-leg.sh`/`parity-guard.sh`, `run-smoke.sh`/`smoke-on-box.sh`, `resolve-legs.sh`/`git-env-scrub.sh` + their shellspec suites, and the CI workflow rewires are all on `devel`. Post-merge review commits renamed two interfaces vs the phase prompts: `run-smoke.sh` takes `--filter` (not `-k`) and the workflows pass `PYTEST_FILTER` (not `PYTEST_K`) — read the prompts as history. Remaining before **Accepted**: the §8 out-of-CI item (2-box concurrent local smoke) — run it, or record it as the documented out-of-CI limitation.
- **Date:** 2026-06-26
