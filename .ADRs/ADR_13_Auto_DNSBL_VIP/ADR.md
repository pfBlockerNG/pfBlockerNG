...
- **Status:** **Accepted** (2026-06-15; implemented 2026-06-03) — the auto-VIP core lifecycle (auto-create on `lo0` → `pfb_dnsvip4` points at it → a listed name sinks → teardown) is automated by `tests/smoke/test_smoke_matrix.py`, green on **CE 2.8 + Plus 26.03** (fanout run 27547011086) (the DNSBL-settings-page persistence is covered by the ADR-14 UI tier). **HA/CARP** sync, uninstall-removes-VIP, conflict-exhaustion, and a real **AAAA** sink to the v6 auto-VIP need hardware/flows the smoke image lacks and are **maintainer-attested**. (Originally Implemented — pending smoke test.)
- **Date:** 2026-06-03
...
| **Lifecycle (create)** | `pfb_manage_dnsbl_vip($mode)`: if `pfb_dnsvip_auto` is on, **ensure** the marked VIP(s) exist at the configured address on `dnsbl_interface` — create + `interface_ipalias_configure()` if absent (idempotent: reuse the existing marked VIP if present), then set `pfb_dnsvip4`/`pfb_dnsvip6` to the `_vip{uniqid}` ids and `write_config`. **Ordering (load-bearing, fixed after the live smoke):** in the main update pass it runs in `sync_package_pfblockerng()` **before `pfb_unbound_python()`**, which bakes the sinkhole IP (`$pfb['dnsbl_vip4']`/`[vip6]`) into the module ini — provision earlier and the module would answer blocks with the *stale* VIP for one pass. `pfb_create_dnsbl($mode)` also calls it (idempotently) for the boot/install path. |
...
| **Scope: IPv6 / recommendation (§7 pivot applied)** | IPv4 auto-created always; IPv6 auto-created **when requested OR when the DNS Resolver listens on IPv6**. If the resolver listens on IPv6, a v6 sinkhole VIP is **wanted** so AAAA blocks are sinkholed: in auto mode it is provisioned with no friction; in manual mode it is a **non-blocking recommendation** — `pfb_global` logs a warning and DNSBL keeps running on IPv4. An earlier revision made it *mandatory* in manual mode (`pfb_validate_vips`/`pfb_global` force-disable); the live smoke (ADR-04) proved that force-disabled DNSBL on every existing manual setup whose resolver listens on IPv6 without a v6 VIP — the common case — so the §7 reject lever was pulled and the hard rule reverted to a warning. Detection via `pfb_unbound_listens_v6()` (resolver active-interface addresses), verified against upstream pfSense source per CLAUDE.md and on-box. |
...
- **v6-mandatory was a behaviour change for existing manual setups** that listen on IPv6 without a v6 VIP — they would have seen a force-disable. **REALIZED in the live smoke (ADR-04): the hard rule force-disabled DNSBL across the whole matrix** (a v4-VIP-only manual setup on an IPv6-listening resolver). The §7 reject/pivot lever was pulled: manual mode is now a **non-blocking warning** (auto mode still provisions the v6 VIP); the smoke matrix passes and existing manual IPv6 users are not broken.
...
5. **IPv6 recommended (not mandatory — §7 pivot):** when the resolver listens on IPv6, a v6 VIP is auto-provisioned in auto mode; in manual mode its absence is a **non-blocking warning** (DNSBL keeps running on IPv4), never a force-disable.
...
- **v6-mandatory too disruptive: PULLED.** Forcing a v6 VIP on existing manual, v6-listening setups broke them (the live smoke force-disabled DNSBL across the whole matrix — a v4-VIP-only setup on a `lo0`-`::1`-listening resolver). Manual mode was softened to a non-blocking warning (auto mode still provisions). Done.
