...
5. **Cache coherence:** the single query-time decision cache (`decisionDB`) is **cleared on the swap** (Context 6); the **allow→block** direction is handled per the Phase-1 decision (delta C-cache flush *or* documented TTL bound) and **block→allow** is immediate (blocks not cached since #43, Context 7); no DNSBL-queries-daemon control collision.
...
- **Cache benchmark:** measure a targeted C-cache **delta-flush** cost vs accepting **TTL-bounded** staleness — typical newly-blocked delta sizes + real feed TTLs.
