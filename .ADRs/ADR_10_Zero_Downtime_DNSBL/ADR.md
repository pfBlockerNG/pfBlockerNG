...
6. **Python-side caches.** (a) `pfb_py_cache.sqlite` (`pfb["pfb_py_cache"]`, `:336`) is the **Reports-tab log** (`dnsblcache` table, `:951`), written *after* a block decision (`:2721`) and **removed at init** today (`:340-342`); it is **never read by the matcher** (not a decision cache). (b) `evaluate_domain` **itself** is cacheless — it re-reads the containers every query (at ≈1 µs/lookup a per-domain memo would be pointless). **BUT its `operate()` wrapper memoizes decisions per query in four module globals reset ONLY in `init()`** (`:403`, `:418-420`): **`excludeDB`** (`:3918`/`:3968` — a name once seen as not-blocked/whitelisted short-circuits the *whole* `evaluate_domain` on later queries), **`dnsblDB`** (`:3924`/`:3976`/`:4004` — a name once blocked reuses the cached block decision), and **`excludeAAAADB`**/**`excludeSS`** (`:3655`/`:3675`, `:3680`/`:3760` — the noAAAA / SafeSearch equivalents). These **are** decision caches. Today's restart-based reload clears them via `init()`; **a no-restart swap that does not re-run `init()` MUST clear them too**, or it serves STALE decisions — a name newly **added** to a list stays unblocked (still in `excludeDB`), a name newly **removed** stays blocked (still in `dnsblDB`). They should also become **sets**, not lists (#26). → Python-side state to handle on a swap is therefore: (i) the Reports sqlite (parity with the restart's `os.remove`, cleared **through `_db_lock`/the db-queue** so it doesn't race `pfb_db_worker`; the async per-block write path is itself benchmarked, §2 Bonus 2), **and (ii) these four query-time decision caches — cleared as part of the atomic swap.**
7. **Unbound's C-side message cache — blocks ARE cached (SYMMETRIC staleness).** The DNSBL block reply is built at **TTL 3600** and does **NOT** set `no_cache_store` — the *only* `no_cache_store=1` in the module is the **SafeSearch CNAME** path (`:2456`), **not** the block path (verified on `next`/`main`/`adr/07`; `rep.security=2` is DNSSEC status, not a cache inhibitor). So DNSBL blocks (and resolved answers) **are stored in Unbound's C-cache** and served from it on repeat queries — **bypassing the python `operate()` decision path** (Unbound answers cache hits before the module chain). Two consequences: **(a) BOTH flip directions are TTL-stale** — a **removed** name keeps serving the cached `0.0.0.0`/VIP, a newly-**added** name keeps serving the cached real answer, each until its TTL; today's reload `dump_cache`/`load_cache`s, so today carries **both** (the original "block→allow is immediate" claim was a misread of `:2456`). **(b) Cache hits under-report** — on a hit only `inplace_cb_reply_cache` → `get_details_reply('cache')` fires (writes `dns_reply.log`, no feed/group), while the feed-attributed `dnsbl.log`/counter runs only on a cache **miss** (`get_details_dnsbl` from `operate()`), so per-feed block stats undercount cached hits (#43). `dnsblDB` is the sidecar that could re-attribute a cached block to its feed (the C-cache can't) — bearing on whether it's kept (#26/#43).
...
Apply DNSBL updates **without restarting Unbound**: the running module rebuilds the matcher structures **on a background thread** off the existing live structures, then **atomically swaps in a single immutable snapshot reference** — GIL-atomic, so every query thread sees either the whole old snapshot or the whole new one, never a mix. PHP stops restarting Unbound for a **data-only** update and instead **atomically publishes** the new manifest and **flips a single sentinel**; a watcher thread inside the module wakes on it and runs the rebuild+swap. The swap also **clears the query-time decision caches** (`excludeDB`/`dnsblDB`/`excludeAAAADB`/`excludeSS` — `init()`-reset today, Context 6), **clears the Python report cache**, and **flushes the Unbound C-cache for the changed names — BOTH newly-blocked and newly-unblocked** (blocks are cached too, Context 7). The premise (shared-interpreter visibility) and the cost (≈2× transient RAM) are **falsified first (Phase 1)**; a config change, a failed build, or a constrained/unsupported box **falls back to today's restart** (fail-safe).
...
| **Cache model — snapshot authority + query-memo invalidation (Bonus 2)** | No per-domain memo is **added** to `evaluate_domain` (at ≈1 µs/lookup it's pointless), so the *matcher structures* have one authority: the swapped snapshot. **BUT `operate()` already memoizes decisions** in `excludeDB`/`dnsblDB`/`excludeAAAADB`/`excludeSS` (reset only in `init()`, Context 6) — these **must be cleared on every swap**, or a newly-added name stays unblocked (`excludeDB`) and a newly-removed name stays blocked (`dnsblDB`). So a swap invalidates **these four query-time caches** (a cheap clear, GIL-atomic; → sets per #26) and handles two non-decision caches below: the **Reports sqlite** (telemetry) and **Unbound's C message cache** (network-latency, not matcher cost — kept). |
...
| **C-cache flip (benchmark-driven)** | **BOTH directions are TTL-stale** (blocks are cached too, Context 7 — the earlier "block→allow immediate" was wrong): allow→block keeps serving the cached real answer, block→allow keeps serving the cached `0.0.0.0`/VIP, each until TTL. Decide by Phase-1/5 measurement: **targeted delta `flush`** of the changed names in **both** directions (correct, one control round-trip each) vs **accept TTL-bounded staleness** (do nothing — **not a regression**: today's restart already preserves the cache, so today is stale both ways too) vs a bounded `flush_zone`. Coalesced + coordinated with the DNSBL-queries-daemon `unbound-control` marker (`:3211`) to avoid control-socket collisions. |
...
- **Cache coherence.** Post-swap the Reports sqlite reflects the new lists (cleared or generation-bumped, parity with restart); **both** flip directions are C-cache-stale (blocks are cached too, Context 7) and handled per the Phase-1 decision (bidirectional delta C-cache flush *or* an accepted, documented TTL bound — the latter is no worse than today, which is stale both ways). No collision with the DNSBL-queries-daemon control socket. The **query-time decision caches** (`excludeDB`/`dnsblDB`/`excludeAAAADB`/`excludeSS`) are **cleared on the swap** (Context 6), so no stale block/allow *decision* survives it on a cache miss (the C-cache flush handles the cached *replies*).
...
- **ADDING a Python decision/resolution cache** — out: the ≈1 µs matcher makes per-domain memoization pointless; we do **not** add one. (The existing `operate()`-level `excludeDB`/`dnsblDB`/`excludeAAAADB`/`excludeSS` query memos are **not** a new cache — they exist today and are simply **cleared on the swap**, Context 6; adding a *further* memo would only create more to invalidate.)
...
- **Cache correctness improves over today** — **both** flip directions stop being TTL-stale (bidirectional delta C-cache flush), which the current restart path (preserve-cache) doesn't fix in either direction.
...
5. **Cache coherence:** the query-time decision caches (`excludeDB`/`dnsblDB`/`excludeAAAADB`/`excludeSS`) are **cleared on the swap** (Context 6); Reports sqlite cleared/generation-bumped on swap (no `_db_lock` race); BOTH flip directions handled per the Phase-1 decision (bidirectional delta C-cache flush *or* documented TTL bound — blocks are cached too, Context 7); no DNSBL-queries-daemon control collision.
...
- Factor the build→install into `rebuild_and_swap()`: run the pure `build()`/`dnsbl_build_from_manifest()`, and **only on success** (a) rebind the single snapshot ref atomically, (b) **clear the query-time decision caches** `excludeDB`/`dnsblDB`/`excludeAAAADB`/`excludeSS` (parity with the `init()` reset at `:403`/`:418-420`, Context 6 — without this the swap serves stale block/allow decisions), (c) reset the Reports sqlite via `_db_lock`/the db-queue — clear (parity with `:340-342`) **or** generation-bump per the Phase-1 benchmark, whichever the Reports UI reads cleaner (Bonus 1), and drop the per-block write path entirely if Phase 1 found it overkill (Bonus 2), (d) re-emit counts. On failure → keep the old snapshot **and** leave the caches intact (fail-closed; mirrors `:558`). Called **synchronously by `init_standard`** for now (no behaviour change at init). Unit-test: success swaps + clears the decision caches + resets the report cache; a build returning `None`/raising leaves the old snapshot AND the caches intact.
...
- The swap is atomic + consistent (no torn read, no half-written-manifest read); caches are coherent (Reports cleared, C-cache flushed in BOTH directions, decision caches cleared).
...
- [ ] **Flip correctness + caches.** A name **queried before** the update then **added** to a feed (allow→block) starts blocking promptly — this needs BOTH the module decision-cache clear (the prior query put it in `excludeDB`, which would otherwise short-circuit the matcher forever) AND the Unbound C-cache handling (the prior real answer, per the Phase-1 delta-flush/TTL-bound decision); a name **queried-and-blocked then removed** (block→allow) resolves promptly — needs BOTH the `dnsblDB` clear (else `operate()` re-blocks it on the next cache miss) AND the C-cache flush of the cached `0.0.0.0`/VIP (blocks ARE cached, Context 7 — it is NOT immediate without the flush); the Reports tab + `pfb_py_count`/`DNSBL_Regex` counts reflect the new lists (incl. cached-hit attribution, #43).
