...
- **Status:** **Proposed** (2026-06-02; **revised 2026-06-04** — see Errata)
- **Date:** 2026-06-02
...
- [ ] **Flip correctness + caches.** A name **queried before** the update then **added** to a feed (allow→block) starts blocking promptly — this needs BOTH the `decisionDB` clear (the prior query memoized it as not-blocked, which would otherwise short-circuit the matcher) AND the Unbound C-cache handling of the prior real answer (per the Phase-1 delta-flush/TTL-bound decision); a name **queried-and-blocked then removed** (block→allow) resolves promptly — this needs **only** the `decisionDB` clear (else `operate()` re-blocks it on the next cache miss), with **no C-cache flush** because blocks are not cached since #43 (Context 7 — block→allow is immediate); the Reports tab + `pfb_py_count`/`DNSBL_Regex` counts reflect the new lists (block stats now fully feed-attributed since blocks always traverse `operate()`, #43).
- [ ] **User custom lists apply instantly (#51).** A name **Lock**ed (user block, allow→block) starts blocking promptly with no restart (needs the `decisionDB` clear + the allow→block C-cache handling); an **Unlock** (block→allow) resolves promptly with no restart (needs only the `decisionDB` clear); the same for "add to whitelist". Unbound's pid is unchanged across the toggle.
