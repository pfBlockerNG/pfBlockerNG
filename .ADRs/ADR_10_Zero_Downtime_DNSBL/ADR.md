...
> 4. **Reports-cache retirement (#1349, 2026-07-15).** The unconsumed
>    `pfb_py_cache.sqlite` writer/table and swap reset were removed. The only
>    Python decision cache a swap handles is `decisionDB`; the resolver/DNSBL
>    counter DB worker and Unbound C-cache rules remain unchanged. Historical
>    reports-cache design text below is superseded by this item.
>
...
6. **Python-side cache.** `evaluate_domain` itself is cacheless — it re-reads the snapshot every query. Its `operate()` wrapper memoizes decisions in one module-global, LRU-capped `decisionDB`. A no-restart swap therefore clears exactly that memo with `decisionDB.clear()`; generation stamps protect against an old-snapshot straggler writing after the clear. Issue #1349 retired the unrelated, unconsumed reports-cache SQLite writer/table.
...
- **Cache coherence.** Only **allow→block** is C-cache-stale (blocks are NOT cached since #43, Context 7) and is handled per the Phase-1 decision, while **block→allow** is immediate. The single query-time decision cache (`decisionDB`) is cleared on the swap, so no stale block/allow decision survives on a cache miss.
...
5. **Cache coherence:** the single query-time decision cache (`decisionDB`) is **cleared on the swap** (Context 6). For **feed/cron updates**, **allow→block** is handled per the Phase-1 decision (delta C-cache flush *or* documented TTL bound). User custom-list, Lock, and whitelist **allow→block** changes always receive an immediate targeted delta flush. **Block→allow** is immediate (blocks not cached since #43, Context 7); no DNSBL-queries-daemon control collision.
...
- **Cache benchmark:** for **feed/cron updates**, measure a targeted C-cache **delta-flush** cost vs accepting **TTL-bounded** staleness — typical newly-blocked delta sizes + real feed TTLs. User custom-list, Lock, and whitelist changes are not part of that choice; verify their mandatory immediate targeted flush separately.
...
- Factor the build→install into `rebuild_and_swap()`: run the pure `build()`/`dnsbl_build_from_manifest()`, and **only on success** (a) rebind the single snapshot ref atomically, (b) clear the single query-time decision cache `decisionDB`, and (c) re-emit counts. On failure, keep the old snapshot and memo intact. Issue #1349 later removed the unconsumed reports-cache reset originally implemented in this phase.
...
- The swap is atomic + consistent (no torn read, no half-written-manifest read); caches are coherent (allow→block C-cache handled, `decisionDB` cleared; block→allow immediate — blocks not cached since #43).
...
- [ ] **Flip correctness + caches.** A name **queried before** the update then **added** to a feed (allow→block) starts blocking promptly — this needs BOTH the `decisionDB` clear and the Unbound C-cache handling of the prior real answer; a name **queried-and-blocked then removed** (block→allow) resolves promptly with the memo clear alone. `pfb_py_count`/`DNSBL_Regex` counts reflect the new lists.
...
- [ ] **Fail-closed.** A deliberately broken/partial manifest does **not** swap — the last-good lists keep serving, the resolver stays up, an error is logged. (The build returns `None`/raises → the watcher keeps the old `_snapshot` and decision memo intact.)
