...
- **Status:** **Implemented (pending live-box smoke)** (2026-06-02; **revised 2026-06-04** — see Errata; implementation complete on `adr/10`, Phases 1–6. Automated guards green and the update/reload contract documented; **flips to Accepted only after the maintainer passes the §7 live-box smoke** on a real pfSense CE box — CI cannot reach Unbound's Python loader or the C-cache.)
- **Date:** 2026-06-02
...
> **Gate: Status flips to Accepted ONLY after every box passes on a live pfSense CE box.** CI cannot reach Unbound's Python loader or the C-cache. Run under continuous DNS traffic. Throughout, watch `/var/log/pfblockerng/*` + the Unbound log, and track Unbound's **pid** (`cat /var/run/unbound.pid`) — an unchanged pid is the no-restart proof.
