...
- **Status:** **Accepted** (2026-07-04) — all four phases implemented via PR #807 (rebase-merged 2026-07-04); §7 acceptance gate met on the live-VM CE 2.8 + Plus 26.03 fan-out (`-k validate_log`, 6 tests incl. the Phase-4 `stage=entries` legs — runs 28701155350 and 28701552450 green on both editions). Review fixes folded in pre-merge (Sonnet 5 adversarial + CodeRabbit): the file-not-found and control-character `pfb_filter()` reject paths honour the `$reject_detail` out-param contract, non-scalar guards in the stats reader, `pfb_reject_detail_summary()` helper. The ZIP post-extraction re-check probing the ORIGINAL archive (a pre-existing no-op inner gate) is tracked in issue #808 for ADR-46 — the smoke inner-stage leg exercises the gz `-bZ` compressed peek instead. (History: Proposed 2026-06-28; facts refreshed 2026-07-03; Phase 4 added + forks resolved 2026-07-04 — §2.1 = option (a), 7z missing-tool = out, buckets = `shape` + `wire_cap`.)
- **Date:** 2026-06-28
