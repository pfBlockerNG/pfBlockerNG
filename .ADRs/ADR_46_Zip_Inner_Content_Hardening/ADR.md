...
- **Status:** **Accepted** (2026-07-04) — implemented via PR #821 (rebase-merged 2026-07-04, direct implementation without phase prompts); §7 gate met on the live-VM CE 2.8 + Plus 26.03 fan-out (`-k adr46`, 4 tests — hostile-member rejects at all THREE disk-writing sites + the benign pass-through pin; final run 28704593724 green). Adversarial review hardened the design pre-merge: the lister discards stderr (a member named `tar: …` can never be dropped as tool noise — a reproduced guard-bypass), returns NULL on a listing failure so the gzip sites fail CLOSED (their ADR-45 probe is only `gunzip -t`, never the inner tar), and the ZIP geoip/top1m site routes an unlistable archive to the guard-free `-xOf` stdout branch. Known name-only limit (symlink members) documented in-code; libarchive's extract-through-symlink refusal covers that vector. Spin-off: issue #823 (pre-existing quoted-path `unlink_if_exists` no-op). (History: Proposed 2026-06-28; §1 inventory corrected 2026-07-03; premise corrected + narrowed to the member guard after issue #808's fix, forks resolved — all three sites guarded — 2026-07-04.)
- **Date:** 2026-06-28
