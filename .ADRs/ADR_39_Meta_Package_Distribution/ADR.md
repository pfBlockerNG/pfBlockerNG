...
| **First-touch** | `add-repo.sh` (no Worker) is a lean bootstrap: install the hook, stage the conf, **run the hook** (`onestart`) to resolve the conf now, verify the marker line, then `pkg update` + verify a package is visible. add-repo.sh does **no detection itself** — the hook is the single source of the resolved conf. The repository copy installs the hook by copying the sibling `scripts/rc.d/pfblockerng_repo_generate.sh` (via `dirname "$0"`). **The published one-file** `site/add-repo.sh` (generated by `gen_landing.py`'s `write_add_repo()`) **embeds the hook via a single-quoted heredoc** (`cat <<'PFB_HOOK_HEREDOC'`) so it is self-contained when piped into `sh` — where `$0` is `sh` and `dirname "$0"` resolution fails. The two paths (`cp` sibling vs. embedded) are selected by `if [ -f "${HOOK_SRC}" ]` in add-repo.sh. |
