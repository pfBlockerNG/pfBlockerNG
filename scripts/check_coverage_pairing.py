...
live under ``src/`` but are verbatim upstream data snapshots or static vendor
text encoding no behaviour, several of them regenerated by the weekly refresh
workflows, so a diff that only refreshes one has nothing to pair a test with.
...
whole path — files under ``src/`` that merely LOOK like data but decide
behaviour (``pfb_dnsbl.*.conf`` unbound config, the feed/ASN catalogs the UI
reads) stay fully gated.
...
_DATA_ONLY = frozenset(
    {
        ...
        # Static upstream licence/usage text shipped verbatim.
...
def _is_neutral(path: str) -> bool:
    ...
    ``_DATA_ONLY`` paths — verbatim upstream data snapshots and static vendor
    text that ship under ``src/`` but encode no behaviour, so a diff that only
    refreshes one has no test to pair with (issue #2132). Membership is by
    WHOLE path, never prefix: a neighbour whose name merely extends an exempt
    one is ordinary src code and stays gated.
    """
