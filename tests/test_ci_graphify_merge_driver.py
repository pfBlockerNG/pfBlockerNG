"""CI writers targeting pfBlockerNG install Graphify; foreign writers do not."""
...
def test_exhaustive_push_matrix_has_graphify_driver_before_each_mutation() -> None:
    ...
    foreign = {("release-published.yml", "sync-ports-fork")}
    assert len(PUSH_JOBS) == len(expected), "duplicate pfBlockerNG writer in the matrix"
    assert _discovered_mutation_jobs() == expected | foreign, (
        "the workflow branch/content mutation inventory changed; classify its repository target"
    )
    assert _discovered_driver_jobs() == expected, (
        "Graphify merge-driver setup differs from the pfBlockerNG-only writer matrix"
    )
