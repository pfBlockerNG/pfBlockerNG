...
_RELEASE_ID = "379867317"
_RELEASE_TAG = "v3.3.7"
_RELEASE_PAYLOAD = '{"id": 379867317, "tag_name": "v3.3.7", "draft": false, "prerelease": false}'
...
def test_the_published_workflow_dispatches_only_with_exact_release_identity() -> None:
    """issue #3004: GitHub suppresses Actions runs for events generated by the
    repository's own GITHUB_TOKEN, and drafts are published by github-actions[bot]
    -- so the implicit `release: published` trigger never fired. The downstream
    workflow must instead be dispatched explicitly with the exact Release
    identity, and must keep no competing event trigger that could ingest the same
    Release twice."""
    ...
    for input_name, end_marker in (
        ...
        input_block = extract_between(text, f"      {input_name}:\n", end_marker)
        assert re.search(r"^        type: string$", input_block, re.MULTILINE), input_block
        assert re.search(r"^        required: true$", input_block, re.MULTILINE), input_block
...
def test_the_classifier_is_fed_the_live_resolved_release() -> None:
    """The dispatch inputs only NAME the Release; the classifier must consume the
    identity the live Release-by-ID resolution actually returned (id, tag,
    prerelease flag), so a lying dispatch input can never reach classification."""
    text = PUBLISHED_WORKFLOW.read_text(encoding="utf-8")
    assert "TAG: ${{ steps.release.outputs.tag }}" in text, text
    assert "RELEASE_PRERELEASE: ${{ steps.release.outputs.prerelease }}" in text, text
    assert "RELEASE_ID: ${{ steps.release.outputs.release_id }}" in text, text
    assert "github.event" not in text, text


def _run_release_resolution_step(
    tmp_path: Path,
    release_id: str,
    release_tag: str,
    payload: str = _RELEASE_PAYLOAD,
    gh_failure: bool = False,
) -> tuple[subprocess.CompletedProcess[str], list[str], dict[str, str]]:
    """Execute the REAL Release-resolution step body under `sh`, with `gh` stubbed
    to serve `payload` (or fail when `gh_failure`) and to record every invocation,
    so tests can assert the step called the API exactly once -- or never, for
    input refused before any network call."""
    script = _step_run_script(_step(_jobs(PUBLISHED_WORKFLOW)["resolve"], "Resolve the dispatched Release"))
    ...
    completed = subprocess.run(  # noqa: S603
        ...
        env={
            ...
            "GH_TOKEN": "stub-token",
            "GITHUB_REPOSITORY": "pfBlockerNG/pfBlockerNG",
            "DISPATCHED_RELEASE_ID": release_id,
    ...
    for line in output_file.read_text().splitlines():
        key, _, value = line.partition("=")
        outputs[key] = value
    return completed, call_log.read_text().splitlines(), outputs


def test_the_resolution_step_resolves_a_published_release_live(tmp_path: Path) -> None:
    completed, calls, outputs = _run_release_resolution_step(tmp_path, _RELEASE_ID, _RELEASE_TAG)
    assert completed.returncode == 0, completed.stdout + completed.stderr
...
def test_the_resolution_step_reads_the_actual_prerelease_flag(tmp_path: Path) -> None:
    """The prerelease flag handed to the classifier comes from the live Release
    payload, never derived from the tag."""
    payload = '{"id": 379867318, "tag_name": "v4.0.0.a1", "draft": false, "prerelease": true}'
    completed, _calls, outputs = _run_release_resolution_step(tmp_path, "379867318", "v4.0.0.a1", payload)
    assert completed.returncode == 0, completed.stdout + completed.stderr
    assert outputs["prerelease"] == "true", outputs


def test_the_resolution_step_refuses_a_draft_release(tmp_path: Path) -> None:
    payload = '{"id": 379867317, "tag_name": "v3.3.7", "draft": true, "prerelease": false}'
...
def test_the_resolution_step_refuses_a_non_numeric_release_id_before_calling_gh(
    tmp_path: Path, release_id: str
) -> None:
    completed, calls, _outputs = _run_release_resolution_step(tmp_path, release_id, _RELEASE_TAG)
    combined = completed.stdout + completed.stderr
    assert completed.returncode != 0, combined
    assert "numeric" in combined, combined
    assert calls == [], calls


def test_the_resolution_step_refuses_an_empty_release_tag_before_calling_gh(tmp_path: Path) -> None:
    completed, calls, _outputs = _run_release_resolution_step(tmp_path, _RELEASE_ID, "")
    combined = completed.stdout + completed.stderr
    assert completed.returncode != 0, combined
    assert "release_tag" in combined, combined
    assert calls == [], calls


def test_the_resolution_step_refuses_a_release_gh_cannot_resolve(tmp_path: Path) -> None:
    completed, calls, _outputs = _run_release_resolution_step(tmp_path, _RELEASE_ID, _RELEASE_TAG, gh_failure=True)
    combined = completed.stdout + completed.stderr
    assert completed.returncode != 0, combined
...
@pytest.mark.parametrize(
    "payload",
    [
        '{"tag_name": "v3.3.7", "draft": false, "prerelease": false}',
        '{"id": "379867317", "tag_name": "v3.3.7", "draft": false, "prerelease": false}',
        '{"id": 379867317, "draft": false, "prerelease": false}',
        '{"id": 379867317, "tag_name": "v3.3.7", "draft": false}',
        "not json at all",
    ],
    ids=["no-id", "string-id", "no-tag", "no-prerelease-flag", "not-json"],
)
def test_the_resolution_step_refuses_a_malformed_release_payload(tmp_path: Path, payload: str) -> None:
    completed, _calls, _outputs = _run_release_resolution_step(tmp_path, _RELEASE_ID, _RELEASE_TAG, payload)
    combined = completed.stdout + completed.stderr
    assert completed.returncode != 0, combined
...
def test_published_workflow_rejects_release_flag_mismatch(
    ...
    assert "steps.release.outputs.prerelease" in step, step
