...
Describe 'check-agent-config-parity.sh'
  ...
  setup() {
    ...
    printf '%s\n' 'HIGH_CLAUDE=fable' 'HIGH_CODEX=gpt-5.6-sol' \
      'MEDIUM_CLAUDE=opus' 'MEDIUM_CODEX=gpt-5.6-terra' \
      'LOW_CLAUDE=sonnet' 'LOW_CODEX=gpt-5.6-luna' > "$work/.agents/model-tiers.conf"
    for role_model in \
      'planner gpt-5.6-sol' \
  ...
  It 'parses model tiers as data and never executes repository-controlled text'
    ...
    printf '%s\n' "HIGH_CODEX=\$(touch $marker)" >> "$work/.agents/model-tiers.conf"
    When run sh "$guard" --root "$work"
    The status should equal 1
