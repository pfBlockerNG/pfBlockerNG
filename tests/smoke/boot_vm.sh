...
cleanup() {
    ...
    # exit (base-not-found, qemu-not-found) as success — a caller could not tell a
    # failed boot from a good one. The happy path execs qemu and never runs this
    # trap. trap - EXIT avoids re-entry from the explicit exit.
    _rc=$?
