...
_BLOCKED_DOMAIN = h.unique_domain("gw")
...
def test_gateway_adapter_roundtrip_live(gateway_vm: SmokeVM) -> None:
    ...
    feed_path = h.write_local_feed(vm, _FEED_NAME, f"{_BLOCKED_DOMAIN}\n")
    spec = h.DnsblCase(
    ...
    _write_field(vm, _CFG_GLOBAL + "/enable_cb", "on")
    _write_field(vm, _CFG_DNSBL + "/pfb_dnsbl", "on")
    ...
    before_runtime = h.dns_probe(vm, _BLOCKED_DOMAIN, "A")
    assert h.is_vip(before_runtime), (
        f"BEFORE gateway writes: expected VIP block for {_BLOCKED_DOMAIN!r}; "
    ...
    after_runtime = h.dns_probe(vm, _BLOCKED_DOMAIN, "A")
    assert h.is_vip(after_runtime), (
        f"AFTER gateway writes: expected VIP block for {_BLOCKED_DOMAIN!r}; "
