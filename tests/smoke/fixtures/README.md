...
## GeoIP fixtures — CSV + binary mmdb, one corpus (issue #1219, #1228)
...
hard-fails on drift — never re-run casually, only when the pin is deliberately bumped). Every
GeoIP fixture file is derived from ONE dataset (244 networks), so seeding both halves via
`helpers.seed_geoip_dataset` drives the *entire* pipeline end to end: the CSVs feed
...
**Real public address space, not RFC 5737/3849.** A GeoIP database can only classify addresses
it actually knows, so the Blocks files carry MaxMind's real test/public IP space (`67.43.156.0/24`,
`81.2.69.0/24`, et al.) — the documented exception to this module's inert-address rule. No
traffic is ever sent to them: they are table rows and `mmdblookup`/pfctl-country-match
arguments only.
