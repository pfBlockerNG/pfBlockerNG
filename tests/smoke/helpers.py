...
def seed_geoip_dataset(vm: SmokeVM, *, timeout: float = 120.0) -> None:
    ...
    issue #1219/#1228: the fixtures under ``tests/smoke/fixtures/GeoLite2-Country*``
    (generated by ``scripts/update-geoip-fixtures.py`` from MaxMind's own public test
    corpus) are ONE consistent dataset seeded twice -- as CSVs for `ugc`
    (pfblockerng_uc_countries() + pfblockerng_get_countries(), dispatcher pfblockerng.php:398-406;
    generator writes in pfblockerng_geoip.inc; no MaxMind key/account gate, unlike the
    DOWNLOAD verbs `dc`/`dcc`/`bu`) AND as the
