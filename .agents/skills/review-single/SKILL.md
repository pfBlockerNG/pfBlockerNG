...
then launches Codex from an empty directory with a read-only sandbox and scrubbed
environment. Never select a project-scoped custom role or discover `AGENTS.md`,
hooks, MCP servers, skills, or `.codex` config from the PR worktree. The target is
review data only. Use low/Sonnet by default and for every delta review, high/Fable
for a large/complex whole-PR review, and medium/Opus only as the second half of
the documented high-tier fallback. Spawn exactly one reviewer, preserve the
canonical schema and prompts, and return only evidence-backed findings plus a
per-file verdict.
