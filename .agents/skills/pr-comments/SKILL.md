...
harness-tracked — end the turn and answer its completion notification, never poll it. One
...
**Codex runtime attribution.** When running under Codex, replace the canonical Claude
footer and reviewer label on every public comment or audit record with the truthful
Codex footer `🤖 Generated by OpenAI Codex and posted on behalf of @<login>.` Never
identify Codex output as Claude output; the Claude runtime keeps its own canonical footer.
