...
- **ACK is only a quota notice** → do **not** drop. `@coderabbitai pause` this PR and every other open product PR (manual `@coderabbitai review` is a no-op unless auto-review is paused — #2430). Apply `cr-hold` to every **other** open PR. Parse "Next review available in", wait `N + 30s` (self-terminating, [`waits.md`](waits.md)), remove `cr-hold` from **this** PR only (keep paused), then **one** `@coderabbitai review`. Never nudge while that countdown is live. Details in coderabbit.md.
