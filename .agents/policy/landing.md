...
- **ACK is only a quota notice** → do **not** drop. Do **not** pause-comment every PR. The `coderabbit-hold` workflow labels open PRs `cr-hold`. Parse "Next review available in", wait `N + 30s`, remove `cr-hold` from **this** PR only, then `@coderabbitai pause` once and `@coderabbitai review` once. Details in coderabbit.md.
