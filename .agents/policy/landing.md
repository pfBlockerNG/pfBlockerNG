...
Each leg reviewer read-only (never edit, commit, or push; scratch under `/var/tmp/agents/<pr>-<leg>/`, removed on return — never `/tmp`, a tmpfs shared by every seat (#3089), and never inside the checkout) and review diff **against a spec** orchestrator build from work item's intent — issue/ADR link, its acceptance criteria / coverage matrix, PR body. Diff-only review cannot catch "asked for ALL X, delivered subset, claimed completeness"; silently narrowed scope is blocking finding. Four parallel leg reviewers split lenses, one each — every lens run, none skipped:

1. **Contract conformance** — map every spec claim to where diff satisfy it; flag unimplemented or narrowed items; enumerate sibling axes from grep (never memory) and flag uncovered rows; flag hardcoded environment-derived literals. Blocking mechanism check (#3203): mechanism absent from the **opening** issue/PR body AND (mutates the host OS, needs developer-host root, or copies a CI runner step onto a laptop) = **blocking**, checkpoint `ready-for-human` — a later comment is not the spec. Unnamed helpers stay `delegation.md` THE BRIEF item 7 ESCALATE.
