...
- **Scope:** vendor-neutral mechanics for landing a pull request — review feedback
  ...
  `pr-comments` skills and the `review-single` reviewer contract (ticket
...
- **Claude:** the adversarial review runs as the committed `review-single` workflow
  (`.claude/workflows/review-single.js` — the executable binding of the reviewer
  contract above; do not restate its prompt); per-finding validation fan-out as the
  `triage-findings` workflow; waits stopped via the task tools. Public-body footer:
