...
- **Guard-erosion check** (`scripts/check_guard_erosion.py`, pre-commit + CI, diff-scoped): a **removed** test declaration under `tests/` — Python `def test_*`, PHPUnit `function test*`, shellspec `It`/`Example` — must be matched by the same name re-declared (edit/move), by an added `successor: <retired name>` line under `tests/`, or by a dated row in `docs/history/retired-tests.md`. Retiring a named-invariant test with neither leaves the invariant unguarded while every gate stays green (issue #2396).
