...
- **Retiring a test needs a successor or a tombstone.** A test asserting a named invariant is that invariant's only mechanical memory, so deleting or renaming one is only done two ways: the assertion moves to a successor test — in a file its runner really collects — carrying a `successor: <retired name>` comment, or the decision is recorded in `docs/history/retired-tests.md` as a dated table row (date, backticked test name, reason nothing replaced it). `scripts/check_guard_erosion.py` blocks a retirement with neither, on the diff, in pre-commit and CI; an edited body keeps its declaration and stays neutral. The class it closes: `--init` was pinned repo-wide, its guard retired days later, the successor never carried the assertion, and the fleet path lost its reaper with every gate green (#2396).
