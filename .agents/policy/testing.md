...
- **Synchronize — duration never an assertion** (#1517, #3157). For controlled synchronous work, call it and assert the returned outcome/state; no settling wait. For asynchronous/external work, consume completion/readiness and return as soon as satisfied; observe exceptions, failed exits and premature EOF promptly rather than waiting for an impossible success. Assert absence only after a completion/drain barrier. Test timeout semantics with controlled clocks/events, never elapsed-wall-clock floors or ceilings, fixed sleeps as coordination, or CI scaling. A generous outer watchdog may only salvage a stuck run: it must interrupt and reap owned work, including on failure, and report `STUCK/ENVIRONMENT` distinctly; an elapsed check after a blocking call cannot do this. Review changed tests and their wait helpers against this contract; inherited/ported violations and green CI are not exemptions.
...
**The host toolchain is NOT automatically CI's** — accepted regression of #2513 (parity was the container's whole job; ADR-47 rests on the same idea). CI grades on Linux with pinned tools: ShellCheck **v0.11.0**, shellspec **0.28.1**, actionlint **1.7.12**, PHP **8.3** and **8.5**, `dash` as the shellspec shell, Python from `uv.lock`. Install those same versions locally; when a claim rides a local run, say which toolchain produced it. **Verdict authority:** where host and CI disagree, CI is the authoritative answer about the verdict on the same commit — green in CI and red here is not a product regression; fix the test guard or install the named package (`libarchive-tools`), never by replacing `/usr/bin/*` on a developer machine. **Mechanism:** CI is never authority over the mechanism — never copy the runner's `dpkg-divert` or `/usr/bin/*` replacement onto a developer host (#3203).

**Divergence shows up as a SKIP, not a red**, which is the harder thing to notice — read the skip list, never just the exit status. Locale data, `file(1)` classification, the PHP build (whether `php://memory` can fail `flock()`), and the invoking uid each decide whether a case runs at all. The `tar` flavour is no longer a PHPUnit skip axis (#3197): on Linux the bootstrap fail-fasts in `tests/php/bootstrap.php` when `bsdtar` is missing — install `libarchive-tools`, never reshape the host. Never read a skip as coverage.
