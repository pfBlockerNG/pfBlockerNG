...
// issue #3194: advisory means advisory. A parse failure carries no
// Diagnostic.actions, so the editor offers the user no affordance for routing a
// line around the Python gate -- there is nothing to route around, because the
// editor cannot block a save at all.
test("lezerErrorDiagnostics attaches no action to a parse failure", () => {
  for (const doc of ["(", "[", "(a"]) {
    const diags = lezerErrorDiagnostics(parsedState(doc));
    assert.ok(diags.length > 0, `expected a diagnostic for ${JSON.stringify(doc)}`);
    for (const d of diags) {
      assert.equal(d.actions, undefined, `actions for ${JSON.stringify(doc)}`);
    }
  }
});
