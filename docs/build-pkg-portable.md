...
`YYYYMMDDHHMMSS.<7-character source SHA>` values — never the static recipe version.
...
| `--pkgversion VERSION` | — | Explicit canonical package version. Required in project mode; Nightly accepts only `YYYYMMDDHHMMSS.<7-character source SHA>`. |
