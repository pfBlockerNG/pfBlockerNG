...
> [!WARNING]
> Package downgrade is unsupported. Back up the pfSense configuration before
> upgrading. Older retained package artifacts may not understand state written
> by this release, so configuration or enforcement may fail; recover by restoring the
> pre-upgrade backup or reinstalling
> the current package and continuing forward.
...
- **Self-hosted package repository** — install and upgrade pfBlockerNG directly
  ...
  **nightly channel** for early testers, **CE- and Plus-aware delivery** of the
  correct package variant for your pfSense edition, and **release retention**
...
- **Safer configuration handling** — a single config access layer with
  behaviour-preserving forward upgrades and canonical current storage.
