...
- **Four-channel catalogue model (issue #2147):** the served tree is
  ...
  ECDSA-signed (issue #2675 — `secp384r1`, private half in the `PFB_PKG_SIGNING_KEY` secret), while
  the client conf is still `signature_type: none`: deployed boxes ignore the added `.sig`/`.pub`
  members and keep trusting HTTPS/TLS to the Pages host until the client half lands; the landing
