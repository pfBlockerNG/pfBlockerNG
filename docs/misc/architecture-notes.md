...
- **Four-channel catalogue model (issue #2147):** the served tree is
  ...
  tuple `scripts/release_version.py:derive_destinations` computes (Stable/Testing/Edge may
  intentionally share the same version and bytes until a later family starts Edge); Nightly is
  untagged and independent. Per `<channel>/<varver>/`, retention keeps the newest
  ...
  equal-priority multi-repo resolution, and the explicit repository-qualified downgrade rule. The arch-less per-varver catalog (issue #1806) is still generated by
...
- **Generators + bootstrap:** `scripts/build-repo-portable.py` (primary catalog gen),
  ...
  project channel repo, above Netgate's 0; `pkg update` + verify). All three `--print-conf`
