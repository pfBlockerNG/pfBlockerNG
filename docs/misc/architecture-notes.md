...
Reload uses (logged `Rebuild`). A Hold row never takes the missing-baseline network fallback:
Hold means
...
`test_adr62_stale_generation_rebuild_hold_row_orig_absent_preserves_staging` sibling
(`.orig` absent → no refetch, byte-identical stale staging preserved); off-appliance reproduction:
...
- **Tier A — `ui_render`** (cheap/hermetic, the **PR gate**): authenticated-HTTP GET of every
  ...
  a page-specific marker present, **and** no new on-box `php_error.log` diagnostic in a
  **gated class** from a pfBlockerNG file. **Never HTTP 200 alone** — body + marker +
  `php_error.log` is the oracle.

  The guest runs a true `E_ALL` (#1218) — deliberately above every level pfSense ships, since
  `rc.php_ini_setup` gives RELEASE `E_ERROR | E_PARSE` and even dev builds
  `E_ALL ^ (E_WARNING | E_NOTICE | E_DEPRECATED)`, so upstream's own maintainer-facing level
  cannot show an `Undefined array key`. The raise is confined to the disposable smoke guest;
  ...
  reports entries it never observed as removal candidates. Never re-mask a class in
  `helpers.STRICT_PHP_ERROR_REPORTING` to quiet a noisy log — that blinds the gate for our
  code too; scope it in the guard.
...
CPU-wildcarded (`FreeBSD:<major>:*`) and the catalog is arch-less (`<channel>/<varver>/` directly,
...
- **Publish pipeline:** `pfBlockerNG/pkg` is the served tree only — GitHub Pages publishes it
  ...
  per-asset `digest`, prepare the dependency flow's sparse ports checkout, and run
  `scripts/publish-pkg-repo.sh`. That script runs the dependency flow first (`scripts/publish_deps.py`,
  issue #2454 — see "Dependency flow" below), then calls
...
- **Dependency flow (issue #2454) — deps are catalogue content, never Release assets.**
  An `extra_pkgs` dependency (e.g. CE's `textproc/py-charset-normalizer`) has identity
  `<py_flavor>-<PORTNAME>-<PORTVERSION>.pkg`, taken from the pinned FreeBSD-ports tree — never
  from its bytes (a rebuild from a new source commit changes `SOURCE_DATE_EPOCH` and thus the
  bytes, which is exactly what tripped the publisher's shared-bytes conflict on nightlies).
  Publishing is therefore split into two independent flows on shared checkouts:
  1. **Dependency flow** — `scripts/publish_deps.py`, run by `scripts/publish-pkg-repo.sh`
     BEFORE the publisher in every publisher-running mode (tagged direct/stage, nightly; never
     promote/discard). Input: the channels being built (nightly → `["nightly"]`; tagged → the
     tag's derived `DESTINATIONS`), the ROUTE matrix (tagged: `ROUTE_MATRIX`; nightly: the
     handoff's own `route_matrix`), the pkg checkout, and a sparse FreeBSD-ports checkout
     (`PORTS_DIR`) at the pinned ports SHA that `scripts/prepare-dep-ports.sh` materialises —
     nightly from the handoff's `ports_sha`, tagged from the canonical assets' own build-record
     provenance (`publish_deps.py --print-ports-sha`). For every ROUTE build row that declares
     an origin × every input channel it checks `docs/<channel>/<varver>/<file>` by presence
     only; every missing (origin, py_flavor, major) is built ONCE via
     `scripts/build-dep-pkg-portable.py`, copied into every missing destination, and each
     touched destination's descriptor is regenerated. The wrapper commits that as its own
     `deps: …` commit at the real location (never under `docs/staging/` — a dependency ahead of
     its project package is harmless), then runs the publisher; one push carries both commits
     inside the same bounded retry loop. Nothing missing → no build, no commit. The flow never
     touches canonical packages, never byte-compares an existing dependency, and does not know
     whether the run is a nightly, a tagged stage, or a republish.
  2. **pfBlockerNG flow** — `publish_release.py` / `publish_nightly.py` are dependency-oblivious:
     `_asset_map` is canonical-only, so `DestinationConflictError` (same name, different bytes)
     is a canonical-package check only; a legacy dependency asset still attached to an older
     Release is verified by `publish_catalogues.verify_run` but never placed. Retention keeps
     `_evict_undeclared_deps` (issue #2402/#2403: a dependency at a destination whose ROUTE row
     no longer declares its origin — category included — is unlinked). The nightly handoff and
     `nightly-result-<major>/result.json` carry `{matrix_row, record, artifact}` only; tagged
     Releases attach one canonical `.pkg` per release row and nothing else (`attach-pkgs`
     uploads `pfSense-pkg-pfBlockerNG-*.pkg` only, the draft healthcheck expects exactly the row
     count and rejects any other row-suffixed asset).
  Testing flows stay self-contained: `release.yml`'s build legs still build each CE row's dep
  `.pkg` and upload it as `pfBlockerNG-relpkg-deppkgs-<Variant>-<pfsense_version>` because the
  run's own smoke/UI gate legs (`pkg_artifact_prefix: pfBlockerNG-relpkg`) install from it, and
  `scripts/smoke-on-box.sh` / `build-pkg-linux.yml` build their own — none of that ever reaches
  a Release or the catalogue.
...
off switch, installed before the package and never removed; the Update page reports the suppression).
The direct `pfblockerng.php tick` verb is
