...
- Bounded firewall-configuration reload (issue #2878): `pfb_filter_reload_cmd()` / `pfb_filter_reload_exec()` in `src/usr/local/pkg/pfblockerng/pfblockerng_apply.inc` wrap `rc.filter_configure_sync` in `timeout --foreground` — the reload starts survivors on purpose (pflog, dpinger, package service restarts), so the reaper would hold the whole budget and kill them on alarm. Output is discarded to `/dev/null` (mwexec's contract) with stdin `/dev/null`; expiry names a 124 and the apply pass is marked pending, never a success report; the budget is the same #2851 operator setting.
