...
- Detached firewall-configuration reload (`pfb_filter_reload_exec()` in `src/usr/local/pkg/pfblockerng/pfblockerng_apply.inc`; owner directive 2026-09-05 supersedes the issue #2878 `timeout --foreground` bound): `rc.filter_configure_sync` is fired `> /dev/null 2>&1 < /dev/null &` and never waited on — pfSense's reload is uncontrollable and signals filterd asynchronously anyway (script exit never meant the rules were live), and the survivors it starts are its own. The only owned failure is the launch: a missing/non-executable script gate returns -1, marks the pass pending, and names itself in the logs.
- Bounded `unbound-control` IPC (issue #2879): `pfb_unbound_control_cmd()` / `pfb_unbound_control_exec()` in `src/usr/local/pkg/pfblockerng/pfblockerng.inc` wrap every request/response control call (`dump_cache`, `status`, `load_cache`, `flush`, `flush_zone`, `flush_stats`) in default (reaper) mode — the control command is a transient helper under `chroot(8)`, so expiry must reap the tree, and `--foreground` would orphan a blocked `unbound-control` still holding the capture pipe. Budget `PFB_UNBOUND_CONTROL_WAIT` = 30 s, the value the sibling resolver waits use; kill grace `PFB_HOOK_KILL_GRACE`. It is also the **whole-batch** ceiling for `pfb_unbound_py_ccache_flush()`, whose per-command deadline is clipped to the time the batch has left, so a name list cannot multiply the wait. 124 is named in both logs, a non-zero reply is named separately, and an expired `dump_cache` discards its partial file so no truncated cache is loaded back.
- `.agents/policy/waits.md` (bounded waits) and Python "no fixed-time waits to coordinate concurrency" rule — agent/test-side analogue: synchronise on signal, bound every wait, never sleep-poll blindly.
