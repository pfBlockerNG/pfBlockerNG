...
- Bounded nested-`pfblockerng.php` re-entry (issue #2016): `pfb_reentry_cmd()` / `pfb_reentry_exec()` in `src/usr/local/pkg/pfblockerng/pfblockerng.inc` use default (reaper) mode, output to a file and stdin `/dev/null`; `pfb_reentry()` in `src/usr/local/pkg/pfblockerng/pfblockerng.sh` uses default (reaper) mode with inherited stdio. Both name a 124 failure; `scripts/check_reentry_bounds.py` keeps new callers on the seam.
