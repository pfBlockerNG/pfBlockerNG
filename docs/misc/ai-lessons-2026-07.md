...
| Review-source policy | CodeRabbit: wait the stated Fair Usage window, then one `@coderabbitai review` (`.agents/policy/coderabbit.md`). Do not drop him because the wait exceeds five minutes. Snyk advisory — only a terminal `failure` where it actually ran gates anything |
