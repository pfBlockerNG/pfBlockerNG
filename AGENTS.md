...
- All repository work happen in dedicated git worktree — cut via
  ...
  branch slug.
- Dev-only classes (ADR text, skills, agent config, docs-only) need no PR; from a
  worktree, fetch/rebase and fast-forward their locally signed commits to `devel`.
- Anything touching `src/`, `tests/`, or CI uses the full PR flow and independent
  review. After all gates, land by explicit GitHub squash or a maintainer-local
  rebase plus signed fast-forward to `devel`; the local path bypasses nothing.
- GitHub merge commits and server-side rebase merges stay disabled; server rebase
  recreates signed commits as unsigned objects. Rebase onto the live base before
  every push, PR, or CI/smoke dispatch; clean the diff before push.
- Push every green commit promptly: code branches stay remote-backed until landing.
- Landing means the change is on `devel` and the PR plus issue are terminal, not
  merely committed. Dev-only work lands at its signed fast-forward push.
