...
| High / medium / low model tier | Read `.agents/model-tiers.conf`: Claude uses `claude-fable-5` / `claude-opus-4-8` / `claude-sonnet-5`; Codex uses `gpt-5.6-sol` / `gpt-5.6-terra` / `gpt-5.6-luna`. Keep the procedure's effort setting independently. |
...
- Use `planner` (high tier), `implementer` (low tier), schema-neutral
  `analyst` / `analyst-high`, and `adversarial-reviewer` (low tier) for the
  standard delegation contract. A
  large/complex whole-PR review uses `adversarial-reviewer-high`; its documented
  fallback pairs the low role with `adversarial-reviewer-medium`. The
