...
5. **Read cost is bounded and asymmetric with the write it replaces.** The line high-water
   ...
   full-file write is the intended trade, not an oversight. **`pfb_count_lines()` counts a
...
Issue #1349 renamed the DNSBL group-statistics helper from
`dnsbl_alias_update()` to `dnsbl_stats_update()` after removing the former helper's
