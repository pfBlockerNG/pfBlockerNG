...
> (`PFB_HOOK_SCRIPT_DIR` = `/usr/local/pkg/pfblockerng/hooks/`, a dedicated dir; named
...
- **Status:** **Accepted** (2026-06-15; implemented 2026-06-02) — the hook mechanism (no-op gate, pre/post fire + full context, trigger values, IP/DNSBL-changed, timeout/non-zero safety, and the webhook-callback shape) is automated by `tests/smoke/test_smoke_hooks.py`, green on **CE 2.8 + Plus 26.03** (fanout run 27547011086). The real **HAProxy** `ipalias_*.lst` emission and **HA/CARP** sync need hardware the smoke image lacks and are **maintainer-attested**. (Originally Implemented — pending smoke test.)
- **Date:** 2026-06-02
...
| Area | Decision |
| --- | --- |
| **Hook model** | A **list of hook entries** in the pfBlockerNG config: `{ script, when: pre\|post, enabled: bool, description, timeout? }` (`script` = a vetted `hook_<when>_*.{sh,py}` basename in the dedicated `hooks/` dir, picked from the folder — not a free-text command). Supports multiple hooks, ordering (list order), and enable/disable. |
...
| **Security / UI** | An **admin-only** settings table (add/edit/enable/reorder/delete; script, when, description, timeout) with help text documenting the env vars. Same root-command trust class as pfSense `shellcmd`; the field is clearly labelled as running as root. |
...
Issue #1349 renamed the non-I/O DNSBL statistics helper from
`dnsbl_alias_update()` to `dnsbl_stats_update()` while retiring the old plaintext
summary writer. Hook semantics are unchanged: changed DNSBL groups are still captured
