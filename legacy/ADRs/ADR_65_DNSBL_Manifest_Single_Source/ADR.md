...
| - | ----- | -------------------- |
| **D1** | `pfb_py_data.txt`/`pfb_py_zone.txt` are no longer written or read; `tld_analysis`'s classification pass and `pfb_dnsbl_py_swap` are removed | The manifest build already produces the production `dataDB`/`zoneDB` (§1.1); this is dead-weight removal. **Net DNS block/resolve decisions are unchanged** — pinned by the Phase-1 decision oracle. Dissolves #1244 + #1245. **Sub-delta (noted post-review, PR #1351):** with the classification loop gone, `tld_analysis`'s retained `dnsbl_alias_update` loop lost its old `$pfb_found` (zone-found) gate and now runs on every `domain_update` pass. The written alias-summary values are identical (they come from the unrelated per-group loop, not classification), and these `{dbdir}/dnsblalias/*` files feed no pf-table reload (grep-confirmed), so the only effect is a summary-file rewrite frequency increase — functionally inert. |
