...
$section->addInput(new Form_StaticText(
	...
	. '<code>PFB_POST_INSTALL</code> &mdash; ' . gettext('<code>1</code> when this run is the reconfigure right '
		. 'after pfBlockerNG was installed or upgraded; otherwise unset.') . '<br />'
	. '<code>PFB_PRE_UNINSTALL</code> &mdash; ' . gettext('<code>1</code> when this run is the teardown while '
		. 'pfBlockerNG is being uninstalled; otherwise unset. Not set when \'Keep enabled during version upgrades\' '
		. 'keeps pfBlockerNG active across an upgrade (nothing is torn down, so no hook fires).') . '<br />'
