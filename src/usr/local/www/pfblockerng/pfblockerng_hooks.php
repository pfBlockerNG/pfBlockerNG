...
$section->addInput(new Form_StaticText(
	...
	. '<code>PFB_PRE_UNINSTALL</code> &mdash; ' . gettext('<code>1</code> when this run is the teardown while '
		. 'pfBlockerNG is being removed, else <code>0</code>. A pfSense version upgrade removes and '
		. 'reinstalls packages, so it also fires this (the removal cannot be told apart from an uninstall).') . '<br />'
