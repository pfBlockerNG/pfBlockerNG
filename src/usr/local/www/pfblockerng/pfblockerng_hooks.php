...
$section->addInput(new Form_StaticText(
	...
	. '<code>PFB_PRE_UNINSTALL</code> &mdash; ' . gettext('<code>1</code> when this run is the teardown while '
		. 'pfBlockerNG is being uninstalled, else <code>0</code>. Stays <code>0</code> when \'Keep enabled during '
		. 'version upgrades\' keeps pfBlockerNG active across an upgrade (nothing is torn down, so no teardown hook '
		. 'fires).') . '<br />'
